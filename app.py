...
def get_db():  # get or create DB connection stored in Flask g
    if 'db' not in g:  # if not already connected
        g.db = psycopg2.connect(
            host=app.config['PGHOST'],  # host param
            database=app.config['PGDATABASE'],  # database param
            user=app.config['PGUSER'],  # user param
            password=app.config['PGPASSWORD'],  # password param
            port=app.config['PGPORT']  # port param
        )
    ...
    if request.method == 'POST':  # handle form submit
        ...
        try:
            ...
            if user and check_password_hash(user['password_hash'], password):  # verify password
                ...
                log_login_attempt(email, True, ip_address)  # log successful login
